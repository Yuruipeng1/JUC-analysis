...
public abstract class AbstractQueuedSynchronizer
    ...
    private void doReleaseShared() {
        ...
        for (;;) {
            ...
            if (h != null && h != tail) {
                ...
                // 要么由于acquire thread获取锁成功而将自己（head后继）设置为新head并且只要head后继不是队尾，
                // 那么新head肯定为SIGNAL。所以设置这种中间状态的head的status为PROPAGATE，让其status又变成负数，这样可能被被唤醒线程检测到。
    ...
    public final void acquireSharedInterruptibly(int arg)
        ...
        if (Thread.interrupted())//上来先检查一下，当前线程的中断状态
            throw new InterruptedException();
        //只要tryAcquireShared返回了 > 0的数，acquireSharedInterruptibly就直接返回了不会阻塞了，
    ...
    final boolean apparentlyFirstQueuedIsExclusive() {
        /**
         * 这只是一种启发式地避免写锁无限等待的做法，它在遇到同步队列的head后继为写锁节点时，
         * 会让readerShouldBlock返回true代表新来的读锁（new reader）需要阻塞等待这个head后继。
         * 但只是一定概率下能起到作用，如果同步队列的head后继是一个读锁，之后才是写锁的话，readerShouldBlock就肯定会返回false了
         *
         */
        Node h, s;
        return (h = head) != null &&
    ...
    public final boolean hasQueuedPredecessors() {
        ...
        //由于是先读取tail，后读取head考虑到这两句代码和enq方法中设置头节点和设置尾节点的两句代码的交叉，
        //只可能出现三种情况，h和t都为null，h和t都不为null，h不为null，t为null，不可能出现h为null，t不为null的情况，
        //这就避免了(s = h.next)这段代码出现空指针异常

        //又因为tail和head都是volatile修饰的所以这两句代码不会出现指令重排的情况

        Node t = tail; // Read fields in reverse initialization order
        Node h = head;
        Node s;
        /**
         * h != t返回false的情况。此时hasQueuedPredecessors返回false。
         *
         * 当h和t都为null，返回false。此时说明队列为空，还从来没有Node入过队。
         * 当h和t都指向同一个Node，也返回false。此时说明队列中只有一个dummy node，那说明没有线程在队列中。
         *
         *
         * h != t返回true，且(s = h.next) == null返回true，直接短路后面。此时hasQueuedPredecessors返回true。
         *
         * 既然h != t返回true，说明h和t不相等，先考虑特殊情况（“head不为null，tail为null”的情况，此时head是空node，next成员肯定为null），
         * 那么说明有一个线程正在执行enq，且它正好执行到if (compareAndSetHead(new Node()))到tail = head;的间隙。但这个线程肯定不是当前线程，
         * 所以不用判断后面短路的s.thread != Thread.currentThread()了，因为当前线程连enq都没开始执行，但另一个线程都开始执行enq了，那不就是说明当前线程排在别人后面了，别的线程马上就要入队了。
         *
         * 既然h != t返回true，说明h和t不相等，再考虑二者都不为null。那此时队列中已经至少有一个等待中的线程了，那说明当前线程肯定排在别人后面了。
         *
         *
         * h != t返回true，且(s = h.next) == null返回false，且s.thread != Thread.currentThread()返回true。此时hasQueuedPredecessors返回true。
         * 如果s.thread != Thread.currentThread()返回false。此时hasQueuedPredecessors返回false。
         *
         * 现在知道head不为null，而且head.next也不为null了（(s = h.next) == null返回false）。我们也知道队列中第一个等待的线程存放在head.next里（注意，head为dummy node，不存放线程），
         * 那么如果head.next的线程不是当前线程，那即说明当前线程已经排在别人线程后面了。如果head.next就是当前线程那就可以去尝试加锁了.
         *
         *
         */
        return h != t &&
    ...
    final boolean transferAfterCancelledWait(Node node) {
        ...
        //通过CAS设置node状态从CONDITION变成0，如果成功了，就将node入队同步队列，然后直接返回true。
        // 注意，这里的CAS操作和transferForSignal里的compareAndSetWaitStatus(node, Node.CONDITION, 0)是一样的，
        // 也就是说：执行signal的线程 和 因中断被唤醒的node线程，在这句compareAndSetWaitStatus(node, Node.CONDITION, 0)这里有竞争关系，
        // 谁竞争赢了，谁才有资格将node入队同步队列enq(node)
        ...
        //如果发现另一个signal线程还没有执行完了enq(node)（通过!isOnSyncQueue(node)条件判断），就必须一直等待，
        // 直到另一个signal线程执行完了enq(node)，然后循环才可以退出。之所以这么等一下，是因为如果不等，
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
        ...
        private Node addConditionWaiter() {
            ...
            // 节点不是CONDITION的，就认为是以后将离开条件队列的节点。
        ...
        private void doSignal(Node first) {
            ...
            // 获得新队头，如果新队头不为null，则继续下一次循环，因为现在一次signal都没有成功
        }
        ...
        public final void await() throws InterruptedException {
            ...
            //进这个if判断说明node线程走的是中断流程，中断流程中不会执行node.nextWaiter == null的操作
