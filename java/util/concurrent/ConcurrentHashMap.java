...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
     ...
     */
    //代表此节点是扩容期间的转发节点，这个节点持有新table的引用。
    ...
    static final int spread(int h) {
        //低16位将受到高16位的扰动(减少hash冲突)，& HASH_BITS后肯定为正数
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        ...
        for (Node<K,V>[] tab = table;;) {
            ...
            //如果发现元素的hash值为其他情况
                    ...
                    if (tabAt(tab, i) == f) {//锁住后，需要检查它是否还在这个索引上
                        ...
                        if (fh >= 0) {
                            ...
                            for (Node<K,V> e = f;; ++binCount) {
                                K ek;
                                //发现有重复key，根据onlyIfAbsent决定是否替换(默认是替换)，
                                if (e.hash == hash &&
                                    ((ek = e.key) == key ||
                ...
                //不等于0，说明,该数组元素有值，执行了新增操作，或替换操作（可能替换没有执行，因为onlyIfAbsent）
                if (binCount != 0) {
                    if (binCount >= TREEIFY_THRESHOLD)
                        treeifyBin(tab, i);//将链表转换成红黑树，但也不一定，要看数组长度是否大于等于64
        ...
        //走到这里，说明当前数组元素上没有值，执行的是else if ((f = tabAt(tab, i = (n - 1) & hash)) == null)这段逻辑，此时binCount为0
     ...
     */
    /**
     * 由于加锁粒度很细（对每个哈希桶加锁），可能存在并发问题，delta代表的计数可能因为并发的插入或删除，
     * 而变得“不正确”。这里是指，clear遍历过程中，别的线程并发的插入或删除了clear遍历过的哈希桶，从而导致不正确。
     */
    public void clear() {
    ...
    static final int resizeStamp(int n) {
        //numberOfLeadingZeros用来计算出第一个1前面的0的个数,而table容量大小为2^4--2^30,所以取值范围为1~27
        //1 << (RESIZE_STAMP_BITS - 1)就是把1左移15位，所以就是1000 0000 0000 0000
        //最大的27都不能占据到第16个bit，所以与1000 0000 0000 0000与一下，不会影响原数的有效bit，只是会把第16个bit置1
        //这个函数的返回值是作为sizectl的高16为使用的，这样sizeCtl的符号位肯定为1，此时sizeCtl肯定为负数

        //并且由于最大的27都不能占据到第16个bit，那么resizeStamp返回值的bit不可能都为1，
        // 那么扩容时，sizeCtl肯定不能为-1，因为-1是每个bit都为1，所以说明sizeCtl为-1是能唯一代表当前table在初始化中的
             ...
             //如果大于等于0，说明sizeCtl暂存着容量
                ...
                try {
                    if ((tab = table) == null || tab.length == 0) {
                        ...
                        table = tab = nt;
                        sc = n - (n >>> 2);//这里是阈值，实际就是 n * 0.75，不过用位运算更快点
                    }
                } finally {
    ...
    //之所以引入counterCells数组，是因为如果每个线程都在CAS修改baseCount这一个int成员的话，必定会造成大量线程因CAS失败而自旋，
    // 从而浪费CPU。现在引入一个counterCells数组，让每个数组元素也来承担计数的责任，则线程CAS修改失败的概率下降了不少
    ...
    private final void addCount(long x, int check) {
        ...
        if ((as = counterCells) != null ||  //如果counterCells成员不为null，短路后面条件
            ...
            if (as == null || (m = as.length - 1) < 0 ||
                ...
                /**
                 * 不管是CAS修改baseCount失败、或者CAS修改某个cell的值失败了，只要是失败了，之后都会执行fullAddCount然后直接return，从而不去执行扩容部分。
                 *
                 * 只有CAS成功了，才有可能去调用扩容部分的代码。
                 *
                 * 这样的做法，可能会导致，该扩容的时候不会扩容。因为毕竟只有CAS直接成功的线程，才可能扩容。
                 * 比如这种场景，线程A CAS成功，此时大小为阈值-1；然后线程B CAS失败，此时大小刚好为阈值，
                 * 但由于失败，不会去扩容；只有等到第3个线程到来，才可能去扩容了。
                 *
                 */
    ...
    final Node<K,V>[] helpTransfer(Node<K,V>[] tab, Node<K,V> f) {
        ...
        if (tab != null && (f instanceof ForwardingNode) &&//检查f是否还是ForwardingNode
            ...
            while (nextTab == nextTable && table == tab &&
                ...
                //transferIndex <= 0，transfer任务已经被领完。
    ...
    private final void transfer(Node<K,V>[] tab, Node<K,V>[] nextTab) {
        ...
        // 检测到每个哈希桶都转移后，还需要从尾到头扫查一遍，是不是每个节点都是ForwardingNode了
        ...
        for (int i = 0, bound = 0;;) {
            ...
            while (advance) {//为true代表当前处理的哈希桶已经处理完毕，需要移动i和bound了
                ...
                //我完成了一次迁移了，就把计数下标-1，因为是从后往前遍历的，如果大于等于bound说明还有剩余数组元素没有迁移完毕，继续迁移
            ...
            //当前遍历哈希桶为null，那好办，设置为ForwardingNode即可
                ...
                //代表这个旧数组的i索引下标节点已经完成了迁移工作
                advance = casTabAt(tab, i, null, fwd);
            //当前遍历哈希桶已经是ForwardingNode，不用处理，说明这个数组索引已经完成了迁移工作
            else if ((fh = f.hash) == MOVED)
                // 设置为true，遍历指针将移动
                ...
                synchronized (f) {//锁住哈希桶的头节点
                    if (tabAt(tab, i) == f) {//需要再次判断f是否还为头节点
                        ...
                        //红黑树分离处理
                            ...
                            ln = (lc <= UNTREEIFY_THRESHOLD) ? untreeify(lo) ://变回链表
                                (hc != 0) ? new TreeBin<K,V>(lo) : t;//hc为0，说明原哈希桶只有低桶元素，就不用再重新组织树结构了
                            hn = (hc <= UNTREEIFY_THRESHOLD) ? untreeify(hi) :
                                (lc != 0) ? new TreeBin<K,V>(hi) : t;//lc为0，说明原哈希桶只有高桶元素，就不用再重新组织树结构了
                            setTabAt(nextTab, i, ln);//低桶的头节点赋值过去
                            setTabAt(nextTab, i + n, hn);//高桶的头节点赋值过去
                            //因为当前数组元素我都已经迁移完毕了，所以这里把当前迁移完毕的数组的slot，当前这个节点设置为fwd，代表已经迁移完毕
    ...
    private final void fullAddCount(long x, boolean wasUncontended) {
        ...
        if ((h = ThreadLocalRandom.getProbe()) == 0) {//探针为0，说明localInit从来没有调用过
            ThreadLocalRandom.localInit();      // force initialization// 先初始化再说
        ...
        for (;;) {
            ...
            if ((as = counterCells) != null && (n = as.length) > 0) {
                //探针取余后的下标的元素为null
                if ((a = as[(n - 1) & h]) == null) {
                    //这个cellsBusy相当于一个独占锁的state，为0说明可以获得锁
                    if (cellsBusy == 0) {            // Try to attach new Cell
                        // 乐观地创建，因为假定获得锁后，该索引位置还是为null
                        CounterCell r = new CounterCell(x); // Optimistic create
                        if (cellsBusy == 0 &&
                            U.compareAndSwapInt(this, CELLSBUSY, 0, 1)) {//尝试获得锁
                            boolean created = false;
                            try {               // Recheck under lock
                                // 获得锁后，还是需要检查该索引位置是否还是为null
                            ...
                            if (created)//如果赋值成功，那么该函数任务完成
                                break;
                            //执行到这里，说明赋值没有成功，因为获得锁后，发现该索引位置却不为null了
                ...
                //代码将不会去U.compareAndSwapLong(a, CELLVALUE, v = a.value, v + x)尝试修改某个cell的值
                // 既然之前的探针冲突了，那么就执行advanceProbe获得下一个探针
    ...
    static class Traverser<K,V> {
        ...
        // 下一个将返回的Node，如果为null，需要去获取
        Node<K,V> next;         // the next entry to use
        // stack是模拟的栈，spare是保存出栈元素以备用（相当于回收站）
        TableStack<K,V> stack, spare; // to save/restore on ForwardingNodes
        // 通过index获取下一个Node
        int index;              // index of bin to use next
        // 最开始的table的遍历索引，在跳转table后index会变化，但baseIndex会一直保存的
