    ...
    private void linkFirst(E e) {
        ...
        for (;;)
            for (Node<E> h = head, p = h, q;;) {
                ...
                else if (p.next == p) // PREV_TERMINATOR // 如果已经脱离队列了，继续外循环，重新读取head
                    continue restartFromHead;
                //执行到这里，说明p已经是first node了。
                else {
                    // p is first node
                    newNode.lazySetNext(p); // CAS piggyback// 这里不需要volatile写动作
                    if (p.casPrev(null, newNode)) {
                        // Successful CAS is the linearization point
                        // for e to become an element of this deque,
                        // and for newNode to become "live".
                        // CAS成功说明p已经成功从first端入队
                        if (p != h) // hop two nodes at a time// p和h的距离至少为1，现在p左边还加了个新节点，所以head需要更新了
            ...
            if (p.item != null || (q = p.next) == null) {
                ...
                if (o != null && p.prev != p && first.casNext(next, p)) {
                    ...
                    if (first.prev == null &&//如果first还是first node
                        ...
                        // 保证从head出发不可能找到 o，或者第二步unlinking跳过的节点
                        updateHead(); // Ensure o is not reachable from head
                        // 保证从tail出发不可能找到 o，或者第二步unlinking跳过的节点
