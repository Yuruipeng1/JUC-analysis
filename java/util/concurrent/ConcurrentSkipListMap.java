...
public class ConcurrentSkipListMap<K,V> extends AbstractMap<K,V>
    ...
    private void initialize() {
        //初始的时候，只有base层和一层index层，而且这两层都只有一个dummy节点作为头节点
        keySet = null;
        entrySet = null;
        values = null;
        descendingMap = null;
    ...
    private Node<K,V> findPredecessor(Object key, Comparator<? super K> cmp) {
        ...
        for (;;) {
            //q和r肯定是处于同一index层的两个相邻index节点(初始时是最高层的第一个和第二个)
                ...
                if (r != null) {//先判断q是否需要右移
                    ...
                    if (n.value == null) {//如果r的node已经被标记为删除
                        if (!q.unlink(r))//断开q与r的连接，进而和r的右节点连接
                            // 如果断开操作失败，说明被其他线程抢先了。整个跳表结构发生变化，需要从新循环
                            break;           // restart
                        // 如果断开操作成功，那么更新r后，开始下一次循环
                        r = q.right;         // reread r
                        continue;
                    }
    ...
    private V doPut(K key, V value, boolean onlyIfAbsent) {
        // 如果最终是增加node，而不是替换node的value。那么z保存新增node引用
        ...
        outer: for (;;) {
            for (Node<K,V> b = findPredecessor(key, cmp), n = b.next;;) {//b是一个<参数key，但接近key的node
                if (n != null) {
                    Object v; int c;
                    Node<K,V> f = n.next;
                    // 如果n在这段时间内被移除掉了，退出内循环重试，这样就可以n = b.next重新读取b的后继
                    if (n != b.next)               // inconsistent read
                        //这种情况说明b的后继被并发修改了
                        break;
                    // n的value为null说明被标记删除，使用helpDelete
                    if ((v = n.value) == null) {   // n is deleted
                        //根据helpDelete的逻辑，如果没有并发线程帮忙，连续两次循环都会执行这句
                        n.helpDelete(b, f);
                        break;
                    }
                    //执行到这里，说明n还是b的后继，且n没有被删除
                    // b被标记删除，或n是一个marker。说明b被删除了(v == n就说明n是个marker，value==this)
                    if (b.value == null || v == n) // b is deleted
                        break;
        ...
        // 如果最高bit和最低bit都是0
        //这段逻辑为新建node创建垂直方向的index，所以如果没有进入分支，那说明这个新建node的垂直方向上将没有index节点
        if ((rnd & 0x80000001) == 0) { // test highest and lowest bits
            /*********************************第二部分*******************************/
            int level = 1, max;
            while (((rnd >>>= 1) & 1) != 0)//每当最高位和最低位中间bit有一个是1,level就++
                ++level;
            Index<K,V> idx = null;
            ...
            if (level <= (max = h.level)) {//如果要建立的index层没超过最大层数
                ...
                //如果要建立的index层超过了最大层数，就建立最大层加1
                level = max + 1; // hold in array and later pick the one to use
                // 从1到level的数组索引，用来放从1到level的新建index,//0元素不使用
                @SuppressWarnings("unchecked")Index<K,V>[] idxs =
                    (Index<K,V>[])new Index<?,?>[level+1];
                for (int i = 1; i <= level; ++i)
                    //自低向上建立index，但每个新建index的前后的right指针还没好,依次放在数组里
                    idxs[i] = idx = new Index<K,V>(z, idx, null);
                for (;;) {
                    h = head;
                    int oldLevel = h.level;//当head的层数
                    // 如果当前层数已经大于等于了想要创建的层数，说明有线程抢先一步增加了head所在层
                    ...
                    if (casHead(h, newh)) {//如果CAS设置head成功，那么此循环任务完成，将退出该循环
                        ...
                        /** 新建节点的比原来高出来的那几层(可能是一层，如果被并发删除了,可能获得的oldLevel已经比max小了,就可能有几层)左右都相连了
                        /   但是新建的index节点们都还没有和前后的index左右相连，为保证第三部分能继续处理，
                        /   则把idx停留在最高的需要左右相连的index节点上，并把level停留在idx所在的层数
                         */
                        break;
                    }
                }
            ...
            splice: for (int insertionLevel = level;;) {
                ...
                for (Index<K,V> q = h, r = q.right, t = idx;;) {
                    ...
                    //一开始，j是最高层数，insertionLevel是最高需要左右相连的index层数
                    if (j == insertionLevel) {
                        if (!q.link(r, t))//执行成功后 q-->t-->r,即t变成q新的后继
                            break; // restart// 连接失败则退出内循环从新开始
                        if (t.node.value == null) {//如果t被标记为删除
                    ...
                    //当t左右相连完成时会下移
                    /**
                     * 执行完--insertionLevel后，insertionLevel + 1 == level成立，
                     * 紧接的if (--j >= insertionLevel && j < level)判断自然也会成立
                     * （此时insertionLevel + 1 == j + 1 == level），也就会去执行t = t.down
                     */
    ...
    final V doRemove(Object key, Object value) {
        ...
        outer: for (;;) {
            for (Node<K,V> b = findPredecessor(key, cmp), n = b.next;;) {
                ...
                Node<K,V> f = n.next;
                //b-->n-->f的结构
                if (n != b.next)    //由于并发，n不是b的后继了    // inconsistent read
                    break;
                //如果n被标记为删除
                if ((v = n.value) == null) {        // n is deleted
                    //如果没有别的线程帮忙，helpDelete会执行两次
                    n.helpDelete(b, f);
                    break;
                }
                //如果b被标记为删除，或者n是个marker
                if (b.value == null || v == n)      // b is deleted
                    break;
                ...
                if (!n.appendMarker(f) || !b.casNext(n, f))
                    //只要有一个操作执行失败，进入此分支
                    //因为findNode会调用helpDelete，helpDelete会做上面的两个操作
    ...
    public ConcurrentSkipListMap() {
        //如果不指定Comparator的话，key就得是Comparator的子类对象
