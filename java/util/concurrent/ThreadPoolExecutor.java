...
public class ThreadPoolExecutor extends AbstractExecutorService {
        ...
        implements Runnable
    {
        ...
        //如果工作线程已经执行过它人生中第一个task(那么state就肯定不是-1了)
        // 那么就中断它
    ...
    private void processWorkerExit(Worker w, boolean completedAbruptly) {
        // 如果runWorker中的循环不是正常结束的，则需要此函数来减小线程数,否则是runWorker里的getTask做了这件事（getTask返回了null）
        ...
        if (runStateLessThan(c, STOP)) {//如果状态为RUNNING或SHUTDOWN，才可能需要补充线程
            ...
            if (!completedAbruptly) {//如果传入Worker是正常结束的
                ...
                if (workerCountOf(c) >= min)//如果当前线程数量满足要求
                    return; // replacement not needed // 那么不需要补充线程，直接返回
            }
            // 1. 如果传入Worker是因为抛出异常而结束，那么肯定补充
            // 2. 如果传入Worker是正常结束，那么视需要而补充(当前线程数量不够最小线程数时，才会去补充)
    ...
    private Runnable getTask() {
        boolean timedOut = false; // Did the last poll() time out? //保存超时poll操作是否操作，
    ...
    public void shutdown() {
        ...
        try {
            ...
            onShutdown(); // hook for ScheduledThreadPoolExecutor // 钩子方法，空实现
        } finally {
            mainLock.unlock();
        }
